...
spm = ['Мгновение – это одна сотая секунды',
       ...
       'В столице Тайвани, городе Тайбэй официально разрешено не использовать число 4. Всё потому, что оно созвучно со словом «смерть». Во многих тайванских домах после третьего этажа сразу идёт пятый',
       'Известный математик современности Стивен Хокинг изучал царицу наук только в школе. Впоследствии, став преподавателем в университете, он просто читал учебник перед занятием, а потом доносил эту информацию студентам',
       ...
       'Все, наверное, слышали историю о студенте, опоздавшем на занятия. Увидев на доске условия задач, он решил, что это домашнее задание. Уравнения показались студенту немного сложнее предыдущих, но, поднапрягшись, он смог найти решение. Так обычный учащийся Джордж Данциг решил задачи, над которыми несколько десятилетий бились величайшие математики мира',
       ...
       'Все слышали о математических трудах Эвклида. Некоторыми его работами пользуются до сих пор. Но какие-либо сведения о личности Эвклида отсутствуют',
       'Существует легенда, что жена Альфреда Нобеля ушла от него к математику. Поэтому Нобель запретил вручать премию его имени представителям этой точной науки',
       ...
       'Автор «Алисы в стране чудес», знаменитый писатель Льюис Кэрролл был еще и неплохим математиком']  # список фактов по математике
...
spi = [
    'Сначала информатикой называлась только техническая область, осуществляющая автоматическую обработку информации при помощи ЭВМ',
    ...
    'Электронная вычислительная машина создавалась на протяжении полугода, а логические схемы в ней были созданы на полупроводниках',
    ...
    'Самой крупной и первой компьютерной атакой была та, которая называлась «Червь Морриса». Она нанесла ущерб примерно 96 миллионов долларов']
    ...
    if event.type == VkEventType.MESSAGE_NEW:
        ...
        if event.from_user and not event.from_me:
            if response == "котики":
                ...
                send_message(vk_session, 'user_id', event.user_id,
                             message='Привет, я GamesBot, напиши "Имя:", а затем как тебя зовут, для того, что бы узнать, что я могу - нажми на кнопку.',
                             keyboard=keyboard)
            elif response == "выход" or response == "пока":
                ...
                ii += 1
                send_message(vk_session, 'user_id', event.user_id,
                             message='Верно! Костя записал IP-адрес школьного сервера на листке бумаги и положил его в карман куртки. Костина мама случайно постирала куртку вместе с запиской. После стирки Костя обнаружил в кармане четыре обрывка с фрагментами IP-адреса. Эти фрагменты обозначены буквами А, Б, В и Г: А: .33, Б: 3.232, В: 3.20, Г: 23, восстановите IP-адрес. В ответе укажите последовательность букв, обозначающих фрагменты, в порядке, соответствующем IP-адресу.',
                             keyboard=keyboard)
            elif response == '44':
                send_message(vk_session, 'user_id', event.user_id,
                             message='Ошибка. Костя записал IP-адрес школьного сервера на листке бумаги и положил его в карман куртки. Костина мама случайно постирала куртку вместе с запиской. После стирки Костя обнаружил в кармане четыре обрывка с фрагментами IP-адреса. Эти фрагменты обозначены буквами А, Б, В и Г: А: .33, Б: 3.232, В: 3.20, Г: 23, восстановите IP-адрес. В ответе укажите последовательность букв, обозначающих фрагменты, в порядке, соответствующем IP-адресу.',
                             keyboard=keyboard)
            elif response == 'гвба':
                ii += 1
                send_message(vk_session, 'user_id', event.user_id,
                             message='Абсолютно верно. В языке запросов поискового севера для обозначения логических операций «ИЛИ» используется символ «|», а для обозначения логической операции «И» — символ «&». Зима & Средиземноморье -340, Зима - 560, Средиземноморье - 780. Какое количество страниц (в тысячах) будет найдено по запросу Зима | Средиземноморье?',
                             keyboard=keyboard)
            elif response == 'гбва':
                send_message(vk_session, 'user_id', event.user_id,
                             message='Неправильно. В языке запросов поискового севера для обозначения логических операций «ИЛИ» используется символ «|», а для обозначения логической операции «И» — символ «&». Зима & Средиземноморье -340, Зима - 560, Средиземноморье - 780. Какое количество страниц (в тысячах) будет найдено по запросу Зима | Средиземноморье?',
                             keyboard=keyboard)
            elif response == '1000':
                ii += 1
                send_message(vk_session, 'user_id', event.user_id,
                             message='Молодец. У исполнителя Альфа две команды. которым присвоены номера: \n 1. Вычти b \n 2. Умножь на 5 (b — неизвестно натуральное число). Выполняя первую из них, Альфа уменьшает число на экране на b, а выполняя вторую, умножает это число на 5. Программа для исполнителя Альфа — это последовательность номеров команд. Известно, что программа 21121 переводит число 2 в число 17. Определите значение b.',
                             keyboard=keyboard)
            elif response == '950':
                send_message(vk_session, 'user_id', event.user_id,
                ...
                send_message(vk_session, 'user_id', event.user_id,
                             message='Напишите "Перевести:", а затем через пробел слово или фразу, которую хотите перевести на английский язык. После нее через пробел пишете либо "ru-en", либо "en-ru", смотря с какого языка вы переводите. Итоговый ввод примерно такой: "Перевести: Яндекс.Лицей ru-en"',
                             keyboard=keyboard)
                ...
                if event.user_id in dname:
                    send_message(vk_session, 'user_id', event.user_id,
                                 message=f'{dname[event.user_id]}, Вы вынуждаете меня с вами поговорить, ну давайте. Для начала давайте узнаем друг друга. Что вы любите? Напишите "люблю:" и  перечисляйте',
                                 attachment=attachment, keyboard=keyboard)
                else:
                    send_message(vk_session, 'user_ifd', event.user_id,
                                 message=f'Вы вынуждаете меня с вами поговорить, ну давайте. Для начала давайте узнаем друг друга. Что вы любите? Напишите "люблю:" и  перечисляйте',
                                 attachment=attachment, keyboard=keyboard)
            elif "имя:" in response:
                kname = response[5:]
                dname[event.user_id] = kname[0].upper() + kname[1:]
                send_message(vk_session, 'user_id', event.user_id,
                             message=f'Рад познакомиться, {dname[event.user_id]}, теперь введи свой возраст в формате "9 лет"')
                ...
                if event.user_id in dname and event.user_id in dage:
                    ...
                    if event.user_id not in a:
                        # в случае если такого айди еще нет, то добавляем в бд его имя и возраст, все это происходит по нажатию моя анкета
                ...
                send_message(vk_session, 'user_id', event.user_id,
                             message='Выберите, что Вы любите из того что я Вам предложил, и я вам скину что-то интересное',
                             keyboard=keyboard)
                ...
                send_message(vk_session, 'user_id', event.user_id,
                             message=f'Сегодня {cl[:10]}, Московское время: {cl[11:]}. Прогноз погоды обновляется примерно раз в 10 минут')
                ...
                send_message(vk_session, 'user_id', event.user_id,
                             message='Я думаю, что основные способности бота ты уж узнал, так что давай перейдем к более интересным темам для разговора.')
